...
def buildCropLocations():
    ...
    ban_coords.extend([bb1, bb2, bb3, bb4, bb5, rb1, rb2, rb3, rb4, rb5])
    ...
    pick_coords.extend([b1, b2, b3, b4, b5, r1, r2, r3, r4, r5])
...
def main():
    ...
    for image in champselect_image_list:
        ...
        icon_splash_width_fixed, icon_splash_height_fixed = findEdges(ban_coords)
        splash_width_fixed, splash_height_fixed           = findEdges(pick_coords)

        createResizedCopies(ban_coords, number_of_copies, icon_splash_width_fixed, icon_splash_height_fixed, cropPath)
...
def removePreviousResults():
    previous_results = [f for f in os.listdir(results_path) if
                        f.endswith(".bmp")]
...
def organiseTemplates(number_of_copies):
    ...
    # sort bans from entire template list, shape into numpy array
    ban_names = ['bb1', 'bb2', 'bb3', 'bb4', 'bb5', 'rb1', 'rb2', 'rb3', 'rb4',
                 'rb5']
...
def template_match(champlist, templates_array, number_of_copies,
        image_filepath, type):
    search_results = []
        ...
        for name in champlist:
            ...
            for file in files_to_check:
                ...
                if (flag):
                    ...
                    # cv.imwrite('Results/'+ current_template2 + '_' + name + '.bmp', compare_image)
                    search_results.append(
                        current_template + ' - ' + name + ' - ' + str(matches))
...
def outputToGoogleSheet(bans_results, picks_results, sheet_name,
        worksheet_name, row_start, col_start):
    scope = ['https://spreadsheets.google.com/feeds',
             'https://www.googleapis.com/auth/drive']
    creds = ServiceAccountCredentials.from_json_keyfile_name(
        'CSA_secrets_GSuite.json', scope)
    client = gspread.authorize(creds)
    sheet = client.open_by_url(sheet_name).worksheet(worksheet_name)
...
def findEdges(list_of_coords):
    ...
    for label, coord_tuple in list_of_coords:
        ...
        if flag:
            print(
                label + ' is cropped differently than other coords')  # time.sleep(1)
...
def createResizedCopies(list_of_crops, number_of_copies, w, h, cropPath):
    for crop, coord in list_of_crops:
        ...
        for i in range(number_of_copies):
            ...
            copy_image = Image.open(
                Path.joinpath(cropPath, str(crop_filename)))
            copy_image = copy_image.resize((width, height))
            copy_image.save(
                str(cropPath) + '/' + crop + '_' + str(counter) + '(s).bmp')
        ...
        for i in range(number_of_copies):
            ...
            copy_image = copy_image.resize((width, height))
            copy_image.save(
                str(cropPath) + '/' + crop + '_' + str(counter) + '(b).bmp')
...
def main():
    ...
    # ---- for only the latest champ select
    champselect_image_list = [f for f in os.listdir(champ_select_path) if
                              f.endswith(".bmp")]
    champselect_image_list = [champselect_image_list[-1]]
    ...
    for image in champselect_image_list:
        ...
        print('                                ... making duplicates')
        icon_splash_width_fixed, icon_splash_height_fixed = findEdges(
            ban_coords)
        splash_width_fixed, splash_height_fixed = findEdges(pick_coords)

        createResizedCopies(ban_coords, number_of_copies,
                            icon_splash_width_fixed, icon_splash_height_fixed,
                            cropPath)
        createResizedCopies(pick_coords, number_of_copies, splash_width_fixed,
                            splash_height_fixed, cropPath)
        ...
        bans_results = template_match(champlist, bans, number_of_copies,
                                      icon_path, 'bans')
        picks_results = template_match(champlist, picks, number_of_copies,
                                       splash_path, 'picks')
        ...
        if google_sheet_name != '':
            print('Sending results to Google Sheet ... ')
            outputToGoogleSheet(bans_results, picks_results, google_sheet_name,
                                worksheet_name, row_start, col_start)
            row_start += row_inc
            col_start += col_inc
