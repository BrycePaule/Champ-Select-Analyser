...
class Scraper:
    ...
    def scrape_splash(self, champ_name, force = False):
        if not force and os.path.isfile(f'./Assets/Splashes_Raw/{champ_name}.bmp'):
            return

        champ_name_adjusted = self.manual_name_adjust_for_website(champ_name)
    ...
    def scrape_icon(self, champ_name, force_scrape = False):
    ...
    @staticmethod
    def manual_name_adjust_for_website(name):
